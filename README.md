...
```json
{
  ...
  "mcServiceName": "MC",
  "serverPropertiesPath": "D:/path/to/your/server.properties"
}
```
...
| `mcServiceName` | string | Windows 服务名称（NSSM 注册时的名称） |
| `serverPropertiesPath` | string | MC `server.properties` 文件路径 |

## 注册为 Windows 服务

使用 [NSSM](https://nssm.cc/) 注册为开机自启服务。以管理员身份运行：
...
项目内附带 `install-service.bat`，可直接右键以管理员身份运行。

常用命令：

```bat
nssm restart MCDashboard   # 重启（修改配置后）
nssm stop MCDashboard      # 停止
nssm remove MCDashboard    # 卸载服务
```
...
├── install-service.bat    # NSSM 服务安装脚本
├── package.json
└── pnpm-lock.yaml
```
...
- **服务管理**: [NSSM](https://nssm.cc/) + Windows `sc` 命令

## 前置要求

- Node.js >= 18
- Windows（服务管理依赖 NSSM / sc）
